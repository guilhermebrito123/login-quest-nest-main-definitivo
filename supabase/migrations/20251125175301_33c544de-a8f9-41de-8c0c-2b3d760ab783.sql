-- Corrigir função gerenciar_ultimo_dia_atividade para não marcar como inativo imediatamente
-- Status só deve mudar para inativo quando a data atual ultrapassar ultimo_dia_atividade
...
BEGIN
  ...
  IF OLD.ultimo_dia_atividade IS NULL AND NEW.ultimo_dia_atividade IS NOT NULL THEN
    ...
    -- Só marcar como inativo se a data atual já passou do ultimo_dia_atividade
    IF v_hoje > NEW.ultimo_dia_atividade THEN
    ...
    ELSE
      -- Manter status atual (vago ou ocupado) se ainda não passou a data
      IF v_colaborador_id IS NOT NULL THEN
  ...
  ELSIF OLD.ultimo_dia_atividade IS NOT NULL AND NEW.ultimo_dia_atividade IS NOT NULL 
    ...
    -- Verificar se deve marcar como inativo
    IF v_hoje > NEW.ultimo_dia_atividade THEN
      -- Desvincular colaborador se houver
      IF v_colaborador_id IS NOT NULL THEN
        UPDATE public.colaboradores
      ...
      NEW.status := 'inativo'::status_posto;
    ELSE
      -- Manter status atual baseado no colaborador
      IF v_colaborador_id IS NOT NULL THEN
        NEW.status := 'ocupado'::status_posto;
      ELSE
        NEW.status := 'vago'::status_posto;
