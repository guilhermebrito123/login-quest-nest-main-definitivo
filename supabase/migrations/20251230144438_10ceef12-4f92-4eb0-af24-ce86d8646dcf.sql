-- Alterar a coluna observacao_pagamento para ser um array do enum observacao_pagamento_type
ALTER TABLE public.diarias_temporarias 
DROP COLUMN IF EXISTS observacao_pagamento;

ALTER TABLE public.diarias_temporarias 
ADD COLUMN observacao_pagamento observacao_pagamento_type[] DEFAULT NULL;

-- Atualizar a função para trabalhar com array
...
BEGIN
  ...
  IF OLD.status IS DISTINCT FROM NEW.status THEN
    ...
    IF NEW.status IN ('Aguardando confirmacao'::status_diaria, 'Confirmada'::status_diaria, 'Aprovada'::status_diaria, 'Lançada para pagamento'::status_diaria) THEN
