...
DECLARE
  ...
  v_conflito_id text;
BEGIN
  -- Validar caracteres permitidos: apenas dígitos, pontos e hífens
  IF new.cpf ~ '[^0-9.\-]' THEN
    RAISE EXCEPTION 'CPF contém caracteres inválidos. Apenas números, "." e "-" são permitidos.';
  END IF;

  -- Validar tamanho máximo de 14 caracteres
  IF length(new.cpf) > 14 THEN
    RAISE EXCEPTION 'CPF deve ter no máximo 14 caracteres.';
  END IF;
