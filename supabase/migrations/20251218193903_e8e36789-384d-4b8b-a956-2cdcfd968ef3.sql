-- Atualizar função para recuperar curriculo_path do storage ao voltar para candidato
...
BEGIN
  ...
  -- Se a nova role é candidato, buscar currículo existente no storage
  IF v_new_role = 'candidato' THEN
    SELECT name INTO v_curriculo_path
    FROM storage.objects
    WHERE bucket_id = 'candidatos-anexos'
      AND name LIKE NEW.id::text || '/curriculo/%'
  ...
  IF v_new_role = 'candidato' THEN
    INSERT INTO public.candidatos (
      id, user_id, nome_completo, email, telefone, celular, cidade, estado, curriculo_path
    ) VALUES (
      NEW.id, NEW.id, v_nome_completo, v_email, v_telefone, v_telefone, 'Não informada', 'Não informado', v_curriculo_path
    )
    ON CONFLICT (id) DO UPDATE SET
      nome_completo = EXCLUDED.nome_completo,
      email = EXCLUDED.email,
      telefone = EXCLUDED.telefone,
      curriculo_path = COALESCE(EXCLUDED.curriculo_path, candidatos.curriculo_path),
      updated_at = now();
  ...
  -- Deletar registro da tabela de origem após criar na tabela de destino
  IF v_old_role = 'candidato' THEN
    DELETE FROM public.candidatos WHERE id = NEW.id OR user_id = NEW.id;
    
  ELSIF v_old_role = 'colaborador' THEN
    DELETE FROM public.colaboradores WHERE id = NEW.id OR user_id = NEW.id;
