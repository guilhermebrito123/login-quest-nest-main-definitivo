...
BEGIN
  ...
  -- 1) status: somente perfis internos (≠ cliente_view) E o chamado precisa ter responsável
  IF NEW.status IS DISTINCT FROM OLD.status THEN
    ...
    IF NEW.responsavel_id IS NULL THEN
      RAISE EXCEPTION 'Não é possível alterar o status de um chamado sem responsável atribuído'
        USING ERRCODE = '42501';
    END IF;
  END IF;
