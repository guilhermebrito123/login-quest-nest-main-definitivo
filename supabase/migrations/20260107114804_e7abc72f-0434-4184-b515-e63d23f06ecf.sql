...
-- Recriar função de log para considerar TODOS os 50 campos de diarias_temporarias
...
BEGIN
  ...
  IF TG_OP = 'UPDATE' THEN
    ...
    -- Campo: pix_alternativo
    IF OLD.pix_alternativo IS DISTINCT FROM NEW.pix_alternativo THEN
      INSERT INTO public.diarias_temporarias_logs (diaria_id, campo, operacao, valor_antigo, valor_novo, usuario_responsavel, operacao_em)
      VALUES (NEW.id, 'pix_alternativo', 'UPDATE', OLD.pix_alternativo, NEW.pix_alternativo, usuario_id, now());
    END IF;
    
    -- Campo: beneficiario_alternativo
    IF OLD.beneficiario_alternativo IS DISTINCT FROM NEW.beneficiario_alternativo THEN
      INSERT INTO public.diarias_temporarias_logs (diaria_id, campo, operacao, valor_antigo, valor_novo, usuario_responsavel, operacao_em)
      VALUES (NEW.id, 'beneficiario_alternativo', 'UPDATE', OLD.beneficiario_alternativo, NEW.beneficiario_alternativo, usuario_id, now());
    END IF;
    ...
    IF OLD.created_at IS DISTINCT FROM NEW.created_at THEN
      INSERT INTO public.diarias_temporarias_logs (diaria_id, campo, operacao, valor_antigo, valor_novo, usuario_responsavel, operacao_em)
      VALUES (NEW.id, 'created_at', 'UPDATE', OLD.created_at::text, NEW.created_at::text, usuario_id, now());
    END IF;
    
    -- Campo: updated_at
    IF OLD.updated_at IS DISTINCT FROM NEW.updated_at THEN
      INSERT INTO public.diarias_temporarias_logs (diaria_id, campo, operacao, valor_antigo, valor_novo, usuario_responsavel, operacao_em)
      VALUES (NEW.id, 'updated_at', 'UPDATE', OLD.updated_at::text, NEW.updated_at::text, usuario_id, now());
    END IF;
